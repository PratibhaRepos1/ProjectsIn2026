    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Manage frequently asked questions for your chatbot.</p>
        </div>
      ...
      <div className="space-y-3">
            ...
            {editId === faq.id ? (
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  {faq.category && (
                    <span className="mt-2 inline-block text-xs bg-brand-50 text-brand-700 px-2 py-0.5 rounded-full">{faq.category}</span>
                  )}
                </div>
                <div className="flex gap-1 flex-shrink-0">
                  <button onClick={() => startEdit(faq)} className="p-1.5 rounded hover:bg-gray-100 text-gray-500">
                    <Pencil size={14} />
                  </button>
                  <button onClick={() => deleteMut.mutate(faq.id)} className="p-1.5 rounded hover:bg-red-50 text-gray-500 hover:text-red-600">
                    <Trash2 size={14} />
                  </button>
                </div>
              </div>
        ...
        {faqs.length === 0 && !adding && (
          <div className="text-center py-12 text-gray-400 text-sm">No FAQs yet. Add your first one above.</div>
        )}
      </div>
    </div>
  )
